...
def close_tribelog():
    if template.check_template_no_bounds("tribelog_check",0.8):
        ...
        if template.window_still_open_no_bounds("tribelog_check",0.8,2):
            ...
            time.sleep(2)
    print('Tribe log closed successfully')
    time.sleep(0.2)
...
def open_structure():
    utils.press_key("AccessInventory")
    if template.template_sleep("inventory",0.7,1.5) == False:
        discordbot.logger("object didnt open retrying")
        utils.press_key("AccessInventory")
        template.template_sleep("inventory",0.7,1.5)
...
def teleport_not_default(arg):
    ...
    # if template.check_template("teleporter_title", 0.7) == False: #timer added incase server lags, taking longer to display tp menu
    #     time.sleep(2)

    count_t = 0
    while template.template_sleep("teleporter_title",0.7,4) == False and count_t <3:
        count_t += 1
        discordbot.logger(f"Teleporter screen not found. Attempt: {count_t}/3")
        time.sleep(0.3)
        check_state()
        utils.pitch_zero()  # changed & added by purge
        utils.set_yaw(settings.station_yaw)  # changed & added by purge
    ...
    utils.turn_up(80)
    #utils.pitch_zero()  # changed & added by purge
    #utils.set_yaw(settings.station_yaw)  # changed & added by purge      THIS LINE AND THE ONE ABOVE ARE TO DO CCC AFTER EVERY TP TO CORRECT THE CAMERA (NO NEED FOR NOW)
    time.sleep(0.4)
    utils.press_key("Run")
