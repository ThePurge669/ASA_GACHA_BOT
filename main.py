...
#added by purge
@bot.tree.command(name="quit", description="Finish current tasks and quit ArkAscended.exe")
async def quit_bot(interaction: discord.Interaction):
    # Inform the user that the bot will finish its current tasks and quit
    await interaction.response.send_message("Finishing current tasks and quitting ArkAscended.exe & Program...")

    # Create a quit task, but only add it if it hasn't been added already
    quit_task = station.quit_arkascended_task(priority=1)

    # Check if the task is already in the queue
    if quit_task not in task_manager.scheduler.waiting_queue.queue:
        task_manager.scheduler.add_task(quit_task)

    # Wait for all running tasks to finish and ensure the quit task is added afterward
    for task in running_tasks:
        if not task.done():
            await task  # Wait for each task to complete

    # Once all tasks are done, the quit task will be executed after the next run cycle of the scheduler
    await interaction.followup.send("All tasks completed. ArkAscended.exe will now quit.")
