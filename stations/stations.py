...
class gacha_station(base_task):
    ...
    def execute(self):
        ...
        if (berry_station and (time_between > 2*60*60)) or time_between > 2*60*60: # if time is greater than 4 hours since the last time you went to berry station
...
class quit_arkascended_task:
    def __init__(self, name="quit_arkascended", priority=0):
        self.name = name
    ...
    def execute(self):
        """
        Terminate ArkAscended.exe and quit the bot.
        """
        success = self.quit_arkascended()
        if success:
            print("ArkAscended.exe has been terminated.")
        else:
            print("ArkAscended.exe is not running or could not be terminated.")

        discordbot.logger(f"\nArkAscended.exe has been terminated.")
        time.sleep(1)
        discordbot.logger(f"\nThe program It's being terminated.")
        time.sleep(5)
        # After ArkAscended.exe is terminated, attempt to close related processes like cmd.exe
        self.quit_task_manager()


    def quit_arkascended(self):
        try:
            # Check for the ArkAscended.exe process and try to terminate it
            for proc in psutil.process_iter(attrs=['pid', 'name']):
                if proc.info['name'].lower() == 'arkascended.exe':
                    proc.terminate()  # Try to terminate the process
                    return True  # If successful, return True
            return False  # If ArkAscended.exe was not found, return False
        except Exception as e:
            print(f"Error terminating ArkAscended.exe: {e}")
            return False
            #added by purge

    def quit_task_manager(self):
        try:
            # Look for any cmd.exe or other processes related to ArkAscended that need to be terminated
            for proc in psutil.process_iter(attrs=['pid', 'name']):
                # You can add other process names if they are related to your ArkAscended setup (e.g., cmd.exe, powershell.exe)
                if proc.info['name'].lower() in ['cmd.exe', 'powershell.exe']:
                    try:
                        proc.terminate()  # Attempt to terminate the process
                        print(f"Terminated {proc.info['name']}")
                        sys.exit()
                    except Exception as e:
                        print(f"Error terminating process {proc.info['name']}: {e}")
        except Exception as e:
            print(f"Error trying to terminate task manager processes: {e}")