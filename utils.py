...
def set_yaw(yaw):
    global current_yaw
    # try:
    #     current_yaw = float(ark.console_ccc()[3])   #PURGE TOOK ALL THIS OUT SO THERE IS NO DOUBLE CCC BECAUSE WE DONT USE HEXAGONAL STATION.
...
def pitch_zero(ccc_data = None):
    ...
    try:
        ...
        current_pitch = 0
        current_yaw = float(ccc_data[3]) #ADDED by purge so we don't have to set the yaw to zero nor do ccc when setting the yaw
    except Exception as e:
